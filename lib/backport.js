...
function send_body() {
    ...
    var code = "$yc_value = function() { return value; };";
    var code2 = "try {"
              // Workaround: Parser does not allow return outside function
              + "    /return/;"
              + "} finally {"
              + "    $yc_value = function () {};"
              + "}";
...
function ctor_body(vardefs, send_body, next_body) {
    return  [ [ "var", vardefs ],
                 [ "return",
                   [ "object",
                     [ [ "send", 
...
function yield_body(body) {
        ...
        MAP(body, function (bpart) {
            ...
            w.with_walkers({
                ...
                "try": function (t, c, f) {
                    ...
                    try_start.jump_implicit();
                    try_start.set();
                    t = MAP(t, walk);
                    finally_after.jump();

                    catch_start.set();
                    c = (c != null ? [ c[0], MAP(c[1], walk) ] : null);
                    finally_after.jump();
                ...
                "yield": function (expr) {
                    ...
                    curBlock.stmt(assign_stat("$y_yielding", ["name", "true"]));
    ...
    var code = "LABEL: do {"
             + "    var CAUGHT = false, YIELDING = false;"
             + "    try {"
             + "        /body/;"
             + "    } catch (EXCEPTION) {"
             ...
             + "        E_LAST = EXCEPTION;"
             + "    } finally {"
             + "        if (!YIELDING && GUARD.length) {"
             + "            PCSTACK.push(COUNTER);"
             + "            COUNTER = GUARD[GUARD.length - 1][CAUGHT ? 2 : 1];"
             + "            continue LABEL;"
             + "        }"
             + "    }"
             + "} while(true)";       
    ...
    var catcher = ast_gen(code, vars, null, {body: looper});
