...
function no_toplevel(ast) {
    return ast[1];
}

function pick_stmt(ast) {
    return ast[0];
}

// Replaces regexps in ast with new ast
function replace_ast_regexp(ast, blocks) {
...
function guard_if() {
    ...
    var code = "if (GUARD.length) {"
             ...
             + "    if (TMP_VAR[2] &&"
             + "        (COUNTER < TMP_VAR[0] || COUNTER > TMP_VAR[1])) {"
    ...
    var ast = parse(str_replace(code, vars));
    ast = replace_ast_regexp(ast, { continue_yield: stmt_continue() } );

    return pick_stmt(no_toplevel(ast));
}
