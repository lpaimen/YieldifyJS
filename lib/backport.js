...
// Also replaces "void" with "return"
        ...
        MAP(body, function (bpart) {
            ...
            w.with_walkers({
                ...
                "break": function (label) {
                    if (label) {
                        break_labels[label].jump();
                    } else {
                        break_stack[break_stack.length - 1].jump();
                    }
                    return this;
                },
                ...
                "continue": function (label) {
                    if (label) {
                        continue_labels[label].jump();
                    } else {
                        continue_stack[continue_stack.length - 1].jump();
                    }
                    return this;
                },
                
                "label": function (name, block) {
                    continue_labels[name] = buyo();
                    break_labels[name] = buyo();
                    continue_labels[name].set();
                    block = walk(block);
                    break_labels[name].set();
                    delete continue_labels[name];
                    delete break_labels[name];
                    return this;
                },
                ...
                "do": function (cond, block) {
                    ...
                    break_stack.push(loop_end);
                    
                    loop_start.set();
                    block = walk(block);
                    cond = walk(cond);
                    loop_start.jump_if(cond);
                    loop_end.set();
                ...
                "stat": function (stat) {
                    stat = walk(stat);
                    return curBlock.stmt([this[0], stat]);
                }
            }, function () {
                return walk(bpart);
            });
