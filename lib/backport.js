...
function ast_ctor_body(vardefs, send_body, next_body) {
    return  [ [ "var", vardefs ],
                 [ "return",
                   [ "object",
                     [ [ "send", 
...
function ast_assign_stat(name, value) {
    return  [ "stat",
               [ "assign",
                 true,
                 [ "name", name ],
...
function backport_yield_function(name, args, body) {
    ...
    if (body[0][0] == "var") {
        ...
        vardefs = MAP(body.shift()[1], function(def) {
            if (def[1]) {
                body.unshift( ast_assign_stat(def[0], def[1]) );
            }
            return [def[0], null];
        });
    }
    ...
    var newbody = ast_ctor_body(vardefs,
                                ast_send_body(),
                                ast_yield_body(body));
